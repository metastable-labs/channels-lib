...
export default class Launchbox {
  ...
  public async getChannelSocialCapital(channelName: string, token: `0x${string}`): Promise<number> {
    try {
      ...
      const tokenHolders = participants.filter((participant) => {
        return participant.tokenBalances.some(tokenBalance => parseFloat(tokenBalance.amount) > 0 && tokenBalance.address.toLowerCase() === token.toLowerCase()
        );
      });
  ...
  */
  public async getChannelParticipants(channelName: string, limit?: number): Promise<Participant[]> {
    try {
      const response: ChannelParticipantsResponse = await fetchQuery(getChannelParticipantsQuery(channelName, limit));

      const participants: Participant[] = response.data.FarcasterChannelParticipants.FarcasterChannelParticipant.map(participantData => {
